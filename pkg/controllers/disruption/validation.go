...
func (e *EmptinessValidator) validateCandidates(ctx context.Context, candidates ...*Candidate) ([]*Candidate, error) {
	...
	validatedCandidates, err := revalidateTargetCandidates(ctx, e.cluster, e.kubeClient, e.recorder, e.clock, e.cloudProvider, e.filter, GracefulDisruptionClass, e.queue, candidates)
	if err != nil {
...
func (c *ConsolidationValidator) validateCandidates(ctx context.Context, candidates ...*Candidate) ([]*Candidate, error) {
	...
	validatedCandidates, err := revalidateTargetCandidates(ctx, c.cluster, c.kubeClient, c.recorder, c.clock, c.cloudProvider, c.filter, GracefulDisruptionClass, c.queue, candidates)
	if err != nil {
		return nil, fmt.Errorf("constructing validation candidates, %w", err)
	}
