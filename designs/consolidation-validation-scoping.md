...
Integration tests T1 through T7 exercise the full scheduling pipeline (real scheduler, real candidate filtering, real validation). Each injects a specific cluster state change during the 15-second validation window.

The emptiness validator shares `revalidateTargetCandidates` but skips `validateCommand` (empty nodes have no pods to simulate). It handles partial candidate loss differently: it proceeds with surviving candidates because each empty node is independently deletable. This is pre-existing behavior. The emptiness path is untested for the scoping change but exercises the same candidate re-check logic.

## Known Limitation: Capacity Displacement

The scoping changes handle cases where unrelated state changes are clearly separable. There is a subtler case they do not handle.

An unrelated pending pod can consume capacity on an existing node during the simulation, pushing a candidate pod onto a NewNodeClaim. The filter correctly identifies this NewNodeClaim as relevant (it contains the candidate's pod), and rejection proceeds as before.
...
This limitation exists today and this change does not alter it. Fixing it would mean changing what pods enter the simulation, which is orthogonal to scoping what differences count as invalidating.
